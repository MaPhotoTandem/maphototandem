...
export default function AdminLoginPage() {
  ...
  // Sauf si ?role=photographe ou ?role=gestion (changement de rôle volontaire)
  useEffect(() => {
    const params = new URLSearchParams(window.location.search)
    const forcedRole = params.get('role') as Role | null

    // Si un rôle est forcé via URL, afficher le login sans auto-redirect
    ...
    // Session gestionnaire active → dashboard gestion
    ...
    // Session photographe active → dashboard photographe
  ...
  async function handleGestionLogin(e: React.FormEvent) {
    ...
    const res = await fetch('/api/admin/ventes', {
      headers: { 'x-manager-password': gestionPw },
    })
  ...
  async function handleLogin(e: React.FormEvent) {
    ...
    const res = await fetch('/api/upload', {
      ...
      body: JSON.stringify({ location, date: 'test', envol: 'test', filename: 'test.jpg' }),
    })
